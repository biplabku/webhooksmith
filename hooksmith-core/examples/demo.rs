//! End-to-end demo: real axum receiver + hooksmith sender + signature verification.
//!
//! What this proves:
//!   1. WebhookEngine sends events with HMAC-SHA256 signatures
//!   2. hooksmith-axum VerifiedWebhook extractor verifies them on the receiving side
...
use hooksmith_axum::{VerifiedWebhook, WebhookSecretLayer};
use serde_json::json;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::fmt()
        .with_env_filter("hooksmith=info,demo=info")
        .without_time()
        .init();

    println!("\n╔══════════════════════════════════════╗");
    println!("║         hooksmith  demo              ║");
    println!("╚══════════════════════════════════════╝\n");

    // ── 1. Start axum receiver ────────────────────────────────────────────────
    println!("[1] Starting axum webhook receiver (hooksmith-axum)...");
