...
use webhooksmith::{error::HooksmithError, UpdateEndpoint, WebhookEngine};
use serde_json::json;
use sqlx::PgPool;
...
async fn partial_update_leaves_other_fields_unchanged(pool: PgPool) {
    ...
    let original = engine
        .register_with(webhooksmith::NewEndpoint {
...
async fn update_rejects_malformed_url_even_with_insecure_urls_allowed(pool: PgPool) {
    ...
    let err = engine
        .update_endpoint(endpoint.id, webhooksmith::UpdateEndpoint {
        ...
        .unwrap_err();
    assert!(
        matches!(err, webhooksmith::HooksmithError::Config(_)),
    ...
    let err2 = engine
        ...
        .await
        .unwrap_err();
    assert!(matches!(err2, webhooksmith::HooksmithError::Config(_)));
